...
if (isset($_GET['edit_id'])) {
    ...
    $order_date = date('Y-m-d', strtotime($order['created_at'] ?? '')); // assuming you have a 'created_at' or similar date column
$today = date('Y-m-d');

if ($loggedUserRole === 'admin') {
    // Admin can always edit
} elseif ($loggedUserRole === 'manager' || $loggedUserRole === 'staff') {
    if ($order_date !== $today) {
        die("Only today's orders can be edited by $loggedUserRole.");
    }
} else {
...
if (isset($_GET['delete_id'])) {
    ...
    if ($loggedUserRole === 'admin') {
        $canDelete = true;
    } elseif (($loggedUserRole === 'manager' || $loggedUserRole === 'staff') && $order_date === $today) {
        $canDelete = true;
    }
...
if (isset($_POST['submit'])) {
    ...
    if (isset($_POST['edit_order_id']) && $_POST['edit_order_id']) {
        ...
        echo "<p style='color:orange;'>Order #$order_id updated.</p>";
    } else {
        ...
        echo "<p style='color:green;'>Order #$order_id placed successfully.</p>";
    }
                ...
                <tbody>
                    ...
                    ?>
                        <tr>
                            ...
                            <td>
                                <?php 
                                $orderDate = date('Y-m-d', strtotime($o['order_date']));
                                $today = date('Y-m-d');
                                $canEdit = false;

                                if ($_SESSION['role'] === 'admin') {
                                    $canEdit = true;
                                } elseif (in_array($_SESSION['role'], ['manager', 'staff']) && $orderDate === $today) {
                                    $canEdit = true;
                                }
                            ...
                            <td>
                                <a href="print_bill.php?order_id=<?= $o['id'] ?>" target="_blank" class="print-btn">
