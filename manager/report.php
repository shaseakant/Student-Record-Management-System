...
// Stock spends
$todaySpends = $conn->query("SELECT SUM(amount) as total FROM stocks WHERE DATE(last_updated) = '$today'")->fetch_assoc()['total'] ?? 0;
$monthlySpends = $conn->query("SELECT SUM(amount) as total FROM stocks WHERE MONTH(last_updated) = MONTH(CURDATE()) AND YEAR(last_updated) = YEAR(CURDATE())")->fetch_assoc()['total'] ?? 0;
$filteredSpends = $conn->query("SELECT SUM(amount) as total FROM stocks WHERE DATE(last_updated) BETWEEN '$startDate' AND '$endDate'")->fetch_assoc()['total'] ?? 0;
